...
// (one ray into an eye from each marked point of the object, virtual rays traced back, image and its characteristics).
...
(function () {
  ...
  const designSchema = [
    ...
    { id: 'grid.show', type: 'toggle', label: 'Graph-paper grid (1 cm squares)', default: true, group: 'Layout' },
    { id: 'grid.colour', type: 'colour', label: 'Grid lines', default: '#dbeafe', group: 'Colours' },
    { id: 'grid.major', type: 'range', label: 'Heavier grid line every', min: 0, max: 10, step: 1, default: 5, unit: 'cm', group: 'Layout' },
    ...
    { id: 'object.shape', type: 'segmented', label: 'Object', options: [{ value: 'arrow', label: 'Arrow' }, { value: 'F', label: 'Letter F' }], default: 'arrow', group: 'Layout' },
  ...
  const controls = [
    ...
    // hide a ray's line in any mode (the class predicts where it goes); overrides the step and the Show toggles
    { id: 'hide.incident', type: 'toggle', label: 'Incident rays', default: false, group: 'Hide lines' },
    { id: 'hide.reflected', type: 'toggle', label: 'Reflected rays', default: false, group: 'Hide lines' },
    { id: 'hide.virtual', type: 'toggle', label: 'Virtual rays', default: false, group: 'Hide lines', showIf: object }
  ];
  ...
  function create(ctx) {
    ...
    function buildStory() {
      ...
      const numbered = preset.id === 'single-ray' ? [
        ...
        ['incident', 'Light from every point on the object spreads out in every direction. One ray from each marked point is shown reaching the mirror: the incident rays.', { incident: true }],
        ...
        ['image', () => 'Each dashed line leads back to the image of the point it came from. The image is the same distance behind the mirror (' + fmtCm(dist()) + ') as the object is in front.', { image: true }],
    ...
    function update(dt) {
      ...
      ['incident', 'reflected', 'virtual', 'image'].forEach(e => {
        ...
        const p = CHAIN[e];
        if (p && shown(p) && (prog[p] || 0) < 1) { prog[e] = prog[e] || 0; return; }   // waiting: stays hidden, not undefined (which would draw the full ray)
    ...
    function layout() {
      ...
      const pts = sourcePoints(d, [tip[0], baseY], h), n = pts.length;
      const rays = pts.map((P, k) => {
        const E = [eye[0] + es * 0.3, eye[1] + es * 0.6 * (k / (n - 1) - 0.5)];   // inside the pupil, on the side facing the mirror
        const img = [2 * mx - P[0], P[1]], t = (img[0] - mx) / (img[0] - E[0]);
        return { src: P, hit: [mx, img[1] + t * (E[1] - img[1])], end: E, img };
      });
    ...
    function drawSingle(L, d) {
      ...
      if (shown('incident')) {
        R.ray(L.src, L.hit, { colour: d['colour.incident'], width: rw, frac: prog.incident || 0, head, arrowAt: 0.5 });
      ...
      if (shown('reflected')) {
        R.ray(L.hit, L.end, { colour: d['colour.reflected'], width: rw, frac: prog.reflected || 0, head, arrowAt: 0.5 });
    ...
    // letter sized so its ink runs exactly from base to tip; left/right are the ink edges from its centre line, dx/dy where to
    // draw it (left-aligned, alphabetic). Measured by drawing offscreen and scanning pixels: Safari's measureText ink bounds are wrong.
    const glyphCache = {};
    function glyphMetrics(d, h) {
      const shape = d['object.shape'], key = shape + '|' + d['text.font'] + '|' + Math.round(h);
      if (glyphCache[key]) return glyphCache[key];
      g.textWidth(shape, { size: h, weight: '800', font: d['text.font'] });   // sets g.ctx.font the same way g.text will
      const c = document.createElement('canvas'), n = Math.ceil(h * 3), x0 = h, y0 = h * 2; c.width = n; c.height = n;
      const cx = c.getContext('2d'); cx.font = g.ctx.font; cx.textAlign = 'left'; cx.textBaseline = 'alphabetic'; cx.fillStyle = '#000';
      cx.fillText(shape, x0, y0);
      const px = cx.getImageData(0, 0, n, n).data; let minX = n, maxX = -1, minY = n, maxY = -1;
      for (let y = 0; y < n; y++) for (let x = 0; x < n; x++) if (px[(y * n + x) * 4 + 3] > 64) { if (x < minX) minX = x; if (x > maxX) maxX = x; if (y < minY) minY = y; if (y > maxY) maxY = y; }
      if (maxX < 0) return { size: h, dx: 0, dy: 0, left: -h / 4, right: h / 4 };
      const f = h / (maxY + 1 - minY), size = h * f, l = (minX - x0) * f, r = (maxX + 1 - x0) * f, half = (l + r) / 2;
      return (glyphCache[key] = { size, dx: -half, dy: (y0 - (maxY + 1)) * f, left: l - half, right: r - half });
    }
    // where rays leave the object: arrow = tip and base; F = top of the stem, end of the top bar, bottom of the stem
    function sourcePoints(d, base, h) {
      if (d['object.shape'] === 'arrow') return [[base[0], base[1] - h], base];
      const m = glyphMetrics(d, h);
      return [[base[0] + m.left, base[1] - h], [base[0] + m.right, base[1] - h], [base[0] + m.left, base[1]]];
    }
    function drawShape(d, s, base, tip, col, mirrored) {
      const shape = d['object.shape'];
      // arrowHead's point sits `size` beyond its anchor, so anchor it below the tip: rays then leave from the very top
      if (shape === 'arrow') { const hs = d['size.head'] * s * 1.3; g.line(base[0], base[1], tip[0], tip[1] + hs, col, 4 * s); g.arrowHead(tip[0], tip[1] + hs, -Math.PI / 2, col, hs); return; }
      const m = glyphMetrics(d, base[1] - tip[1]), o = { size: m.size, weight: '800', font: d['text.font'], color: col, align: 'left', baseline: 'alphabetic' };
      g.ctx.save(); g.ctx.translate(base[0], base[1]); if (mirrored) g.ctx.scale(-1, 1);
      g.text(m.dx, m.dy, shape, o);   // left-aligned and centred by hand, matching glyphMetrics
      g.ctx.restore();
    }
    ...
    function drawObject(L, d) {
      ...
      if (shown('incident')) {
        L.rays.forEach(r => R.ray(r.src, r.hit, { colour: d['colour.incident'], width: rw, frac: prog.incident || 0, head, arrowAt: 0.5 }));
      ...
      if (shown('reflected')) {
        L.rays.forEach(r => R.ray(r.hit, r.end, { colour: d['colour.reflected'], width: rw, frac: prog.reflected || 0, head, arrowAt: 0.5 }));
      ...
      if (shown('virtual')) {
        L.rays.forEach(r => R.ray(r.hit, r.img, { colour: d['colour.virtual'], width: rw, frac: prog.virtual || 0, arrow: false, dashed: true }));
        if (labelled('virtual') && prog.virtual >= 1) labelAlong(L.rays[1].hit, L.rays[1].img, 0.5, 'virtual rays', false, lab(d, { colour: d['colour.virtual'] }));
      }
      if (shown('image')) g.alpha(prog.image || 0, () => {
        drawShape(d, s, L.imgBase, L.imgTip, d['colour.image'], true);
        L.rays.forEach(r => g.circle(r.img[0], r.img[1], 3.5 * s, d['colour.virtual']));
        if (labelled('image')) {
    ...
    // 1 cm graph paper, anchored on the mirror (fixed) so the object and its image sit on the same squares either side of it
    function drawGrid(d, s, L) {

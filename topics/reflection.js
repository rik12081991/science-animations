...
(function () {
  ...
  function create(ctx) {
    ...
    // letter sized so its cap height runs exactly from base to tip; left/right are the glyph's edges from its centre line
    function glyphMetrics(d, h) {
      const shape = d['object.shape'], o = { size: h, weight: '800', font: d['text.font'], align: 'center', baseline: 'alphabetic' };
      g.textWidth(shape, o); g.ctx.textBaseline = 'alphabetic'; g.ctx.textAlign = 'center';   // sets ctx.font the same way g.text will; bounds are then measured from the centre line
      const m0 = g.ctx.measureText(shape), asc = m0.actualBoundingBoxAscent || h * 0.72, size = h * h / asc, f = size / h;
      return { size, left: -(m0.actualBoundingBoxLeft == null ? m0.width / 2 : m0.actualBoundingBoxLeft) * f, right: (m0.actualBoundingBoxRight == null ? m0.width / 2 : m0.actualBoundingBoxRight) * f };
    }
    ...
    function drawShape(d, s, base, tip, col, mirrored) {
      ...
      const m = glyphMetrics(d, base[1] - tip[1]), o = { size: m.size, weight: '800', font: d['text.font'], color: col, align: 'center', baseline: 'alphabetic' };
